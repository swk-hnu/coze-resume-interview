  ...
  state.flow.thread = [
    {
      ...
      meta: ["支持上传 PDF / DOC / DOCX / TXT", "真实模式需要部署 API 地址和 Token"],
    },
  ];
}
...
const publishedWorkflowClient = {
  ...
  async runWorkflowStream(context) {
    ...
    return requestWorkflowStream(buildCozeApiUrl("/v1/workflow/stream_run"), {
      method: "POST",
      headers: {
        ...workflowHeaders(),
  ...
  async resumeWorkflowStream(context) {
    ...
    return requestWorkflowStream(buildCozeApiUrl("/v1/workflow/stream_resume"), {
      method: "POST",
      headers: {
        ...workflowHeaders(),
        "Content-Type": "application/json",
  ...
  async runWorkflow(context) {
    ...
    const raw = await requestJson(buildCozeApiUrl("/v1/workflow/run"), {
      method: "POST",
      headers: {
        ...workflowHeaders(),
        "Content-Type": "application/json",
  ...
  async resumeWorkflow(context) {
    ...
    const raw = await requestJson(buildCozeApiUrl("/v1/workflows/resume"), {
      method: "POST",
      headers: {
        ...workflowHeaders(),
        "Content-Type": "application/json",
...
async function uploadResumeFile(file) {
  ...
  const result = await requestJson("https://api.coze.cn/v1/files/upload", {
    method: "POST",
    headers: {
...
function formatUploadedFileValue(resumeUpload) {
  ...
  return JSON.stringify({ file_id: resumeUpload.id });
}
